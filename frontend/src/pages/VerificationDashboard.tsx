  ...
  return (
    <div className="space-y-6 md:space-y-8">
      ...
      <div className="bg-white rounded-lg shadow p-4 md:p-6">
        <h1 className="text-2xl md:text-3xl font-bold text-gray-900 mb-2">Verification Dashboard</h1>
        <p className="text-sm md:text-base text-gray-600">Review and verify pending asset registrations</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-6">
        <div className="bg-white rounded-lg shadow p-4 md:p-6">
          <div className="flex items-center">
            ...
            <div className="ml-3 md:ml-4">
              <p className="text-xs md:text-sm font-medium text-gray-600">Pending Assets</p>
              <p className="text-xl md:text-2xl font-bold text-gray-900">{pendingAssets.length}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow p-4 md:p-6 sm:col-span-2 md:col-span-1">
          <div className="flex items-center">
            ...
            <div className="ml-3 md:ml-4">
              <p className="text-xs md:text-sm font-medium text-gray-600">Verifier</p>
              <p className="text-xs md:text-sm font-bold text-gray-900">{account?.slice(0, 8)}...</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow">
        <div className="px-4 md:px-6 py-4 border-b border-gray-200">
          <h3 className="text-base md:text-lg font-medium text-gray-900">Pending Assets</h3>
        </div>
          ...
          {pendingAssets.length === 0 ? (
            ...
            <div className="space-y-4">
              {pendingAssets.map((asset) => (
                <div key={asset.id} className="border border-gray-200 rounded-lg p-4 md:p-6 hover:shadow-md transition-shadow">
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 gap-3">
                    <div className="flex-1">
                      <h4 className="text-base md:text-lg font-semibold text-gray-900">{asset.assetType}</h4>
                      <p className="text-xs md:text-sm text-gray-500">ID: {asset.assetId}</p>
                    </div>
                    <div className="flex items-center space-x-2 md:space-x-4">
                      <span className={`px-2 md:px-3 py-1 text-xs font-medium rounded-full ${getStatusColor(asset.status)}`}>
                        {getStatusLabel(asset.status)}
                      </span>
                      <button
                        onClick={() => openVerificationModal(asset)}
                        className="px-3 md:px-4 py-2 text-xs md:text-sm font-medium text-white bg-blue-600 rounded-md hover:bg-blue-700 transition-colors"
                      >
                        Review
                      </button>
                    </div>
